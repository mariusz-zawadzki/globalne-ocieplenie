...
function dataForCityAndYear(city: string, year: string): Promise<string[]> {
    ...
    return docs.then(function (value: DocumentSnapshot) {
        ...
        if(temperatures){
            return DAYS.map((day)=>{return temperatures.get(day)}) as string[];
        }
        return result;
    });
}
...
export class Cities extends React.Component<CitiesProps, {}, any> {
    ...
    render(): React.ReactElement<any, string | React.JSXElementConstructor<any>> | string | number | {} | React.ReactNodeArray | React.ReactPortal | boolean | null | undefined {
        const options = this.props.cities.map((name) => {
            return <option key={"city" + name} value={name}>{name}</option>
        });
        return <select onChange={(e) => this.props.updateCity(e.target.value)}>
            {options}
        </select>;
    }
}
...
export class Chart extends React.Component<ChartProps, ChartState, any> {
    ...
    componentDidUpdate(prevProps: Readonly<ChartProps>, prevState: Readonly<ChartState>, snapshot?: any): void {
        if(this.state.city && this.state.year){
            dataForCityAndYear(this.state.city, this.state.year).then((data)=>{
                this.echarts_react.getEchartsInstance().setOption({
                    series: [{
    ...
    render(): React.ReactElement<any, string | React.JSXElementConstructor<any>> | string | number | {} | React.ReactNodeArray | React.ReactPortal | boolean | null | undefined {
        return <div>
            <div>
                <Cities cities={["KIELCE-SUKÓW","LODZ"]} updateCity={(city)=> this.updateChart(city, this.state.year)} />
                <Cities cities={["1984","1985","1988","2014","2019"]} updateCity={(year)=> this.updateChart(this.state.city, year)} />
            </div>
            <ReactEcharts ref={(e) => {
